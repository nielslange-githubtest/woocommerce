...
class WC_Discounts {
	...
	protected function validate_coupon_expiry_date( $coupon ) {
		if ( $coupon->get_date_expires() && apply_filters( 'woocommerce_coupon_validate_expiry_date', time() > $coupon->get_date_expires()->getTimestamp(), $coupon, $this ) ) {
			/* translators: %s: coupon code */
			throw new Exception( esc_html( sprintf( __( 'Coupon "%s" has expired.', 'woocommerce' ), $coupon->get_code() ) ), 107 );
		}
	...
	protected function validate_coupon_minimum_amount( $coupon ) {
		...
		if ( $coupon->get_minimum_amount() > 0 && apply_filters( 'woocommerce_coupon_validate_minimum_amount', $coupon->get_minimum_amount() > $subtotal, $coupon, $subtotal ) ) {
			...
			throw new Exception( esc_html( sprintf( __( 'The minimum spend for coupon "%1$s" is %2$s.', 'woocommerce' ), $coupon->get_code(), wc_price( $coupon->get_minimum_amount() ) ) ), 108 );
		}
	...
	protected function validate_coupon_product_ids( $coupon ) {
		if ( count( $coupon->get_product_ids() ) > 0 ) {
			...
			if ( ! $valid ) {
				/* translators: %s: coupon code */
				throw new Exception( esc_html( sprintf( __( 'Sorry, coupon "%s" is not applicable to selected products.', 'woocommerce' ), $coupon->get_code() ) ), 109 );
			}
		}
	...
	protected function validate_coupon_sale_items( $coupon ) {
		if ( $coupon->get_exclude_sale_items() ) {
			...
			if ( ! $valid ) {
				/* translators: %s: coupon code */
				throw new Exception( esc_html( sprintf( __( 'Sorry, coupon "%s" is not valid for sale items.', 'woocommerce' ), $coupon->get_code() ) ), 110 );
			}
		}
	...
	protected function validate_coupon_excluded_product_ids( $coupon ) {
		...
		if ( count( $coupon->get_excluded_product_ids() ) > 0 ) {
			...
			if ( ! empty( $products ) ) {
				...
				throw new Exception( esc_html( sprintf( __( 'Sorry, coupon "%1$s" is not applicable to the products: %2$s.', 'woocommerce' ), $coupon->get_code(), implode( ', ', $products ) ) ), 113 );
			}
		}
	...
	protected function validate_coupon_excluded_product_categories( $coupon ) {
		if ( count( $coupon->get_excluded_product_categories() ) > 0 ) {
			...
			if ( ! empty( $categories ) ) {
				...
				throw new Exception( esc_html( sprintf( __( 'Sorry, coupon "%1$s" is not applicable to the categories: %2$s.', 'woocommerce' ), $coupon->get_code(), implode( ', ', array_unique( $categories ) ) ) ), 114 );
			}
		}
