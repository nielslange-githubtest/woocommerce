...
class WooPaymentsRestControllerTest extends WC_REST_Unit_Test_Case {
	...
	public function test_onboarding_business_verification_check_po_eligible() {
		...
		$this->mock_woopayments_service
			->expects( $this->once() )
			->method( 'get_onboarding_kyc_po_eligible' )
			->with( $country_code, $self_assessment )
			->willReturn( $po_eligibility_response );

		// Act.
		$request = new WP_REST_Request( 'POST', self::ENDPOINT . '/onboarding/step/' . $step_id . '/check/po_eligible' );
		$request->set_param( 'location', $country_code );
		...
		$this->assertArrayHasKey( 'success', $data );
		$this->assertTrue( $data['success'] );
		foreach ( $po_eligibility_response as $response_entry => $response_value ) {
			$this->assertArrayHasKey( $response_entry, $data );
			$this->assertSame( $response_value, $data[ $response_entry ] );
		}
	}

	/**
	 * Test onboarding business verification check PO eligible with invalid location.
	 *
	 * @dataProvider provider_invalid_location_provider
	 *
	 * @param string $location The location to test.
	 */
	public function test_onboarding_business_verification_check_po_eligible_with_invalid_location( string $location ) {
		// Arrange.
		$step_id      = WooPaymentsService::ONBOARDING_STEP_BUSINESS_VERIFICATION;
		$country_code = 'US';
		$this->mock_onboarding_details( $country_code );
	...
	/**
	 * Test onboarding business verification check PO eligible with exception.
	 */
	public function test_onboarding_business_verification_check_po_eligible_with_exception() {
		// Arrange.
		$step_id      = WooPaymentsService::ONBOARDING_STEP_BUSINESS_VERIFICATION;
		$country_code = 'US';
		$this->mock_onboarding_details( $country_code );
